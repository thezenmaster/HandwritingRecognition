...
namespace BackPropagation
{
    class BackPropagation
    {
        ...
        public static void Train(int numInput, int numHidden, int numOutput)
        {
            try
            {
                ...
                while (ctr < 1500 && error > 0.1)
                {
                    ...
                    //if (ctr < files.Length)
                    //{
                    //    var testObj = files[ctr];
                    //    name = testObj.Name;
                    //    using (FileStream stream = testObj.Open(FileMode.Open))
                    //    {
                    //        Bitmap image = new Bitmap(stream);
                    ...
                    //        image.Dispose();
                    //        yValues = nn.ComputeOutputs(xValues);
                    //        tValues = GenerateTargetValues(numOutput, name);
        ...
        public static void Test(int numInput, int numHidden, int numOutput)
        {
            try
            {
                int totalWeights = (numInput * numHidden) + (numHidden * numOutput) + numHidden + numOutput;
                ...
                foreach (var file in files)
                {
                    using (FileStream stream = file.Open(FileMode.Open))
                    {
                        ...
                        {
                            result += yValues[i];
                            //PrintResultValue(yValues[i], intervals);
                        }
                        result = (double) (result/yValues.Length);
                        PrintResultValue(result, intervals);
                    }
                    Console.WriteLine();
                }
                Console.ReadLine();
        ...
        public static void PrintResultValue(double value, List<Interval> intervals)
        {
            foreach (var interval in intervals)
            {
                if (value >= interval.Min && value <= interval.Max)
                {
                    Console.Write("{0} ", interval.Letter);
                    return;
                }
            }
        }
